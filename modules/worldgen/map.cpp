...
void world_map::function_create_map(content::content_manager &manager) {
    for (int k = 0; k < var_mapsize; ++k) {
        for (int l = 0; l < var_mapsize; ++l) {
            chunk::border_chunk borderChunk = function_determine_border(k,l);
...
chunk::border_chunk world_map::function_determine_border(int k, int l){
    if (k == 0){
        if (l == 0){
            return chunk::border_chunk::upper_left_corner;
        }else if(l != var_mapsize -1 ){
            return chunk::border_chunk::up;
        }else{
            return chunk::border_chunk::upper_right_corner;
        }
    }else if(k>0 && k != var_mapsize -1){
        if (l == 0){
            return chunk::border_chunk::left;
        }else if(l == var_mapsize -1){
            return chunk::border_chunk::right;
        }
    }else if (k == var_mapsize -1){
        if(l == 0){
            return chunk::border_chunk::bottom_left_corner;
        }else if (l > 0 && l != var_mapsize -1){
            return chunk::border_chunk::bottom;
        }else if (l == var_mapsize -1){
            return chunk::border_chunk::bottom_right_corner;
        }
    }else{
        return chunk::border_chunk::none;
    }
    return chunk::border_chunk::none;
}
